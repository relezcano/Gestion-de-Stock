    ...
    if (id_or_name != ""){
      ...
      $.ajax({
        ...
        data: {idOrName: id_or_name},
          success: function(data){
            if(data == 101){
              alert("\tIngrese un id de cliente existente!\t");
              $(w).val("");
              $(document).find("div[id='clientData']").html("");
            }else{
              $(w).val("");
...
function endSellProcess(action){
  ...
  //Si la accion que se desea realizar es la de cargar se procede a hacer la misma.
  if(action == 'load'){
    ...
    //Se hacen variables de los datos que se necesitan para la carga.
    ...
    //Se hacen arrays vacios para los productos y las cantidades que se venderan.
    var id_Prod     = [];
    var cant        = [];
    var numRows     = document.getElementById("myTable").rows.length;

    //Se agrega en un array los productos junto con sus cantidades.
    for(i=0; i<numRows; i++){
    ...
    //Si no estan seteados los valores necesarios para la carga se le avisa al usuario y no se realiza la misma.
    if((price_Total != "" && sobrante != "" && id_Prod != "" && cant != "") && (price_Total != undefined && sobrante != undefined && id_Prod != undefined && cant != undefined)){

      //Ventana emergente para confirmar la carga de la venta.
      var loadConf = confirm("\t¿Esta seguro que desea cargar la venta?\t");
      if(loadConf == true){
        $.ajax({
          ...
          success: function(data){
            console.log(data);
            if(data == '100'){
              alert("\tSe ha cargado exitosamente la Venta.\t");
              window.close();
            } else if (data == '101') {
      ...
      alert("\tDebe completar los datos necesarios para continuar\t");
    }
  //Si la accion que se desea realizar es cancelar la venta, se procede a realizar el proceso de cancelado.
  }else if (action == 'cancel') {
    //Mensaje emergente para confirmar la cencelacion de la venta.
    var cancelConf = confirm("\t¿Esta seguro que desea cancelar la venta?\t");
    if(cancelConf == true){
      window.close();
    }
  }
